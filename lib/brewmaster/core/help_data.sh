...
Usage: brewmaster [upgrade] [packages...] [options]
       ...
       brewmaster report
       brewmaster completion [bash|zsh|fish] [--shell=NAME]
       brewmaster help [command]
...
UPGRADE (default command)
  ...
  Before executing, candidates are always shown for review. With fzf installed
  every candidate starts selected: deselect what you don't want (tab: toggle,
  ctrl-a: all, ctrl-d: none), then enter; with --check-deps each row shows its
  risk score. Without fzf: a table plus a single [y/N] for the whole batch.
  --dry-run and --yes both skip this (--dry-run shows the table and stops;
  --yes upgrades all).
...
SHELL COMPLETION
  completion                 Report whether completion for your shell is installed
                             and set up to load, with the snippet to add if not.
  completion <bash|zsh|fish> Print that shell's completion script, for
                             source <(brewmaster completion zsh) or a git checkout.
                             e.g. brewmaster completion --shell=zsh

  Flags (apply to completion):
    --shell=NAME             Report for NAME (bash|zsh|fish) instead of $SHELL.

  The tap formula installs completions for all three shells. "Configured" is
  read from your rc files: brewmaster cannot see the live shell, and never
  writes to them.
