...
TBD — one contract for every `fzf`-backed multi-select in the CLI
(`upgrade`'s review gate, `cleanup --interactive`), implemented by
`ui_select` in `lib/brewmaster/core/ui.sh`: a marker distinct from the
cursor pointer, inline height, a key header generated from the same bind
list the picker applies, a caller-chosen preselect mode, and a
documented degradation path when `fzf` is absent or too old to support a
requested bind.
