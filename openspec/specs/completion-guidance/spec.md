...
TBD — a read-only diagnostic and printer for the shipped shell
completions: `brewmaster completion` reports whether the user's shell
(`$SHELL` or `--shell=NAME`) has the completion script installed and its
rc files set up to load it, printing the snippet to add when not, and
`brewmaster completion <bash|zsh|fish>` prints the script itself. It
never writes to rc files or completion directories.
