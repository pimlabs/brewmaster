...
TBD: defines per-command help dispatch (`brewmaster help <command>`) and
the bare `brewmaster help` / `--help` / `-h` grouped reference, both
sourced from the shared help data module `lib/brewmaster/core/help_data.sh`,
so command-specific usage, flags, and examples stay in one place instead of
duplicated inline heredocs.
