...
TBD — caches the Homebrew Cellar root path per command invocation and
derives per-package Cellar paths from it, avoiding repeated `brew --cellar`
and `brew list` calls during `cleanup`, `bloat`, and `why`.
