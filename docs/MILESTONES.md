# Milestone History — M6 onward

> As-built notes for completed milestones: the scope as planned, what
> actually shipped and why it differed, files touched, and the acceptance
> criteria as verified. M0–M5 (the frozen v1 contracts that `tests/`
> reference) are in [`ARCHIVE_ROADMAP.md`](ARCHIVE_ROADMAP.md). Status and
> conventions live in [`../ROADMAP.md`](../ROADMAP.md); each milestone's
> full proposal, design, specs and tasks are under
